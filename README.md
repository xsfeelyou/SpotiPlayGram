...
4. Post **two** messages in the channel in the following order:
   - **First message** (`TG_MEDIA_MESSAGE_ID`) — send any image **as a compressed photo** (not as a file) and add any text as a caption. For example, you can send `default.png` from the `session/spotify/` folder. This message will be used for the album cover. The Telegram API only allows replacing media in a message that already contains an image, so a plain text message will not work
...
SpotiPlayGram uses **two separate Telethon sessions**:
 ...
 - **User session** (`session/telegram/user.session*`) — requires full Telegram authorization (phone number and cloud password, if set). Used for actions that need a real user
 - **Bot session** (`session/telegram/bot.session*`) — uses `TG_BOT_TOKEN`. Used for the control panel (`/start`) and channel updates

To run on another device (PC, server, VPS, etc.) after the initial authorization, copy the files `session/telegram/user.session*`, `session/telegram/bot.session*` and `session/spotify/.cache`.
...
- Delete the `session/spotify/.cache` file and restart the application — authorization will be repeated via browser
...
**Session file errors**
 - If the application cannot connect to Telegram or throws session errors, delete `session/telegram/user.session*` and `session/telegram/bot.session*`, then restart the application to re-authorize
