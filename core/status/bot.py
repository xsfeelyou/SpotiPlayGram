...
from logger import _safe_log_info, _safe_log_error, _normalize_log_arg
from constants import (
    ...
    INFO_PATTERNS_CONFIRM_ADD_FEAT,
    INFO_PATTERNS_CONFIRM_ADD_TAG,
    INFO_PATTERNS_CONFIRM_DELETE_FEAT,
    INFO_PATTERNS_CONFIRM_DELETE_TAG,
    INFO_PATTERNS_ENTER_ADD_FEAT,
    INFO_PATTERNS_ENTER_ADD_TAG,
    INFO_PATTERNS_ENTER_DELETE_FEAT,
    INFO_PATTERNS_ENTER_DELETE_TAG,
    INFO_PATTERNS_MENU_FEAT,
    INFO_PATTERNS_MENU_TAG,
    INFO_PATTERNS_SAVED_WILL_APPLY_FEAT,
    INFO_PATTERNS_SAVED_WILL_APPLY_TAG,
    ...
    REASON_EMPTY_TELEGRAM_RESPONSE,
    VALUE_NOT_SET,
)
...
class BotState:
    ...
    last_spotify_keepalive_attempt: datetime = datetime.min
    last_genius_keepalive_attempt: datetime = datetime.min
    tg_backoff_seconds: int = 0
    tg_next_retry_ts: float = 0.0
    tg_last_disconnect_log_ts: float = 0.0
    tg_was_disconnected: bool = False
    shutting_down: bool = False
    update_loop_last_log_ts: float = 0.0
    update_loop_last_log_sig: str = ""
...
def mark_shutting_down() -> None:
    ...
    state.tg_was_disconnected = False
    state.tg_backoff_seconds = 0
    state.tg_next_retry_ts = 0.0
    state.tg_last_disconnect_log_ts = 0.0
...
async def delete_all_messages(user_client, channel_id):
    ...
    try:
        ...
        return
    if messages is None:
        _safe_log_error(ERROR_MESSAGE_DELETE, REASON_EMPTY_TELEGRAM_RESPONSE)
        return
...
async def setup_bot_commands(bot, user):
    ...
    try:
        from lyrics.patterns.base import add_patterns, remove_patterns, pattern_exists, _normalize_item
    except ImportError:
        add_patterns = None
    ...
    async def show_mistral_menu(event):
        ...
        try:
            ...
            await bot.send_message(
                chat,
                ERROR_MISTRAL_FETCH_MODELS.format(_normalize_log_arg(e)),
    ...
    async def main_menu_router(event):
        ...
        try:
            ...
            if state.current_menu == "patterns" and state.patterns_kind in ("tag", "feat") and text in (BTN_PATTERNS_ADD, BTN_PATTERNS_DELETE):
                ...
                if state.patterns_kind == "tag":
                    enter_text = INFO_PATTERNS_ENTER_ADD_TAG if state.patterns_action == "add" else INFO_PATTERNS_ENTER_DELETE_TAG
                else:
                    enter_text = INFO_PATTERNS_ENTER_ADD_FEAT if state.patterns_action == "add" else INFO_PATTERNS_ENTER_DELETE_FEAT
                await event.respond(
                    enter_text,
                    parse_mode="html",
                    buttons=[[Button.text(BTN_BACK, resize=True), Button.text(INFO_MAIN_MENU, resize=True)]],
            ...
            if state.awaiting_patterns_confirm and state.current_menu == "patterns" and state.patterns_kind in ("tag", "feat"):
                if (state.patterns_action == "add" and text == BTN_PATTERNS_CONFIRM_ADD) or (state.patterns_action == "delete" and text == BTN_PATTERNS_CONFIRM_DELETE):
                    ...
                    if state.patterns_kind == "tag":
                        saved_text = INFO_PATTERNS_SAVED_WILL_APPLY_TAG
                    else:
                        saved_text = INFO_PATTERNS_SAVED_WILL_APPLY_FEAT
                    await event.respond(
                        saved_text,
                        parse_mode="html",
                        buttons=[[Button.text(BTN_PATTERNS_ADD, resize=True), Button.text(BTN_PATTERNS_DELETE, resize=True)], [Button.text(BTN_BACK, resize=True), Button.text(INFO_MAIN_MENU, resize=True)]],
            ...
            if state.awaiting_patterns_input and state.current_menu == "patterns" and state.patterns_kind in ("tag", "feat") and text:
                ...
                if state.patterns_kind == "tag":
                    confirm_template = INFO_PATTERNS_CONFIRM_ADD_TAG if state.patterns_action == "add" else INFO_PATTERNS_CONFIRM_DELETE_TAG
                else:
                    confirm_template = INFO_PATTERNS_CONFIRM_ADD_FEAT if state.patterns_action == "add" else INFO_PATTERNS_CONFIRM_DELETE_FEAT

                await event.respond(
                    confirm_template.format(formatted),
                    parse_mode="html",
...
async def patched_update_channel(user_client, bot_client, spotify_client):
    ...
    try:
        ...
        patterns_base = None
    apply_pending_reload = getattr(patterns_base, "apply_pending_reload", None) if patterns_base is not None else None
    state = _state
    st = get_settings()
    ...
    async def _ensure_tg_connected(*, force: bool = False) -> tuple[bool, int]:
        now = time.monotonic()
        user_ok = _is_connected(user_client)
        bot_ok = _is_connected(bot_client)

        if state.shutting_down:
            if user_ok and bot_ok:
                if state.tg_was_disconnected:
                    state.tg_was_disconnected = False
                    state.tg_backoff_seconds = 0
                    state.tg_next_retry_ts = 0.0
                    state.tg_last_disconnect_log_ts = 0.0
        ...
        if user_ok and bot_ok and not force:
            if state.tg_was_disconnected:
                state.tg_was_disconnected = False
                state.tg_backoff_seconds = 0
                state.tg_next_retry_ts = 0.0
                state.tg_last_disconnect_log_ts = 0.0
                _safe_log_info(INFO_TG_RECONNECTED)
            return True, 0

        state.tg_was_disconnected = True

        if state.tg_backoff_seconds <= 0:
            state.tg_backoff_seconds = 5
        if force:
            state.tg_next_retry_ts = 0.0
        if state.tg_next_retry_ts <= 0.0:
            state.tg_next_retry_ts = now

        if now >= state.tg_next_retry_ts:
            try:
                if not _is_connected(user_client):
                    await user_client.connect()
            except (RPCError, ConnectionError, asyncio.TimeoutError, OSError, RuntimeError, ValueError, TypeError):
                pass
            try:
                if not _is_connected(bot_client):
                    await bot_client.connect()
            except (RPCError, ConnectionError, asyncio.TimeoutError, OSError, RuntimeError, ValueError, TypeError):
                pass

            if _is_connected(user_client) and _is_connected(bot_client):
                state.tg_was_disconnected = False
                state.tg_backoff_seconds = 0
                state.tg_next_retry_ts = 0.0
                state.tg_last_disconnect_log_ts = 0.0
            ...
            state.tg_backoff_seconds = min(max(state.tg_backoff_seconds * 2, 5), 300)
            state.tg_next_retry_ts = now + float(state.tg_backoff_seconds)

        retry_in = max(1, int(state.tg_next_retry_ts - now))
        if now - state.tg_last_disconnect_log_ts >= 60.0:
            try:
                _safe_log_error(ERROR_TG_DISCONNECTED_RETRY, retry_in)
            except (ValueError, TypeError, OSError):
                pass
            state.tg_last_disconnect_log_ts = now
        ...
        while True:
            ...
            try:
                if not state.status_enabled:
                    ...
                    if (current_time - state.last_spotify_keepalive_attempt).total_seconds() >= 30:
                        state.last_spotify_keepalive_attempt = current_time
                        try:
                            await spotify_client.current_playback()
                        except (aiohttp.ClientError, asyncio.TimeoutError, OSError):
                            pass
                    if st.enable_genius and (current_time - state.last_genius_keepalive_attempt).total_seconds() >= 30:
                        state.last_genius_keepalive_attempt = current_time
                        try:
                            ...
                            if token:
                                ...
                                async with session.get("https://api.genius.com/search", params={"q": "ping"}, headers=headers, timeout=timeout):
                                    pass
                        except (aiohttp.ClientError, asyncio.TimeoutError, OSError):
                            pass
                    ...
                    if track_info["track_id"] is not None:
                        ok, retry_in = await _ensure_tg_connected()
                        if not ok:
                            await asyncio.sleep(max(sleep_seconds, retry_in))
                            continue
                        await _apply_default_state(
                            bot_client=bot_client,
                            write_track_info=write_track_info,
                    ...
                    if wrote:
                        ...
                        try:
                            if callable(apply_pending_reload):
                                feat_applied, tag_applied = apply_pending_reload()
                                if feat_applied or tag_applied:
                                    parts = []
                                    if tag_applied:
                ...
                try:
                    ...
                    err_text = ""
                sig = f"{type(e).__name__}:{err_text}"
                if (now - state.update_loop_last_log_ts >= 60.0) or (sig != state.update_loop_last_log_sig):
                    _safe_log_error(ERROR_UPDATE_LOOP, e)
                    state.update_loop_last_log_ts = now
                    state.update_loop_last_log_sig = sig
                try:
                    if isinstance(e, (ConnectionError, OSError)) or "Request was unsuccessful" in err_text or "Cannot send requests while disconnected" in err_text:
                        ok, retry_in = await _ensure_tg_connected(force=True)
                        if not ok:
                            sleep_seconds = max(sleep_seconds, retry_in, 5)
                except (RPCError, ConnectionError, asyncio.TimeoutError, OSError, RuntimeError, ValueError, TypeError):
                    pass
