...
async def _read_json_payload(resp) -> Optional[Dict[str, Any]]:
    ...
    t = txt.lstrip()
    if not t:
        _safe_log_error(ERROR_GENIUS_HTTP, "JSON", REASON_EMPTY_JSON_RESPONSE)
        return None
    try:
        data = json.loads(txt)
    ...
    async def _run_search_phase(queries: List[str]) -> None:
        ...
        for res in results:
            ...
            for song in res:
                ...
                if sid is None:
                    continue
                if sid in matched_song_ids:
                    matched = True
                elif sid in rejected_song_ids:
                    matched = False
                else:
                    matched = any(
